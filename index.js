...
(async () => {
    ...
    async function enviarRespostasCorrigidas(respostasAnteriores, taskId, answerId, headers) {
        ...
        try {
            ...
            notifications.error("Erro ao enviar respostas corrigidas.", 5000);
        }
    }
    ...
    // ==================== SISTEMA DE FALLBACK PARA AS APIs ====================
    const BACKEND_APIS = [
        'https://trollchipss-tarefas.vercel.app',
        'https://trollchipss-tarefas.onrender.com'
    ];
    let currentBackendIndex = 0;

    async function callBackendEndpoint(endpoint, payload) {
        const apiUrl = BACKEND_APIS[currentBackendIndex];
        ...
        try {
            ...
            // Trocar para a próxima API em caso de erro
            currentBackendIndex = (currentBackendIndex + 1) % BACKEND_APIS.length;
            throw error;
        }
    }

    // Expor funções para a interface
    window.automationSystem = {
        login: async (ra, password) => {
            try {
                const response = await callBackendEndpoint('auth', { ra, password });
                return response;
            } catch (error) {
        ...
        getPendingTasks: async (authToken, nick) => {
            try {
                const response = await callBackendEndpoint('tasks/pending', { auth_token: authToken, nick });
                return response;
            } catch (error) {
                return { success: false, message: error.message };
            }
        ...
        getExpiredTasks: async (authToken, nick) => {
            try {
                const response = await callBackendEndpoint('tasks/expired', { auth_token: authToken, nick });
                return response;
            } catch (error) {
                return { success: false, message: error.message };
            }
        ...
        processTasks: async (authToken, tasks, timeMin, timeMax, isDraft = false) => {
            try {
    ...
    console.log('✅ Sistema de automação carregado com interceptação de requisições!');
})();
