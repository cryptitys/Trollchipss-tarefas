...
def auth():
    ...
    try:
        ...
        print(f"DEBUG /auth: login OK para RA {ra}, usuário {data.get('nick')}")
...
def process_task():
    ...
    try:
        task_id = task.get("id")
        print(f"DEBUG /task/process: iniciando task_id={task_id}")
        ...
        print(f"DEBUG /task/process: task_id={task_id} vai simular {processing_time}s")
...
def complete_tasks():
    ...
    print(f"DEBUG /complete: recebidas {len(tasks)} tarefas para processar")
    ...
    return jsonify({"success": True, "results": results, "message": f"{len(tasks)} tarefas enviadas para processamento"})
