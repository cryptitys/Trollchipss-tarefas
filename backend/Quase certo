...
def rooms():
    ...
    try:
        ...
        if r.status_code != 200:
            return jsonify({"success": False, "message": "Erro ao buscar salas", "detail": r.text}), r.status_code

        return jsonify({"success": True, "rooms": r.json()})
...
def tasks():
    ...
    try:
        r = requests.get(f"{API_BASE_URL}/tms/task/todo",
                         ...
                         params=params)
        if r.status_code != 200:
            return jsonify({"success": False, "message": "Erro ao buscar tarefas", "detail": r.text}), r.status_code

        return jsonify({"success": True, "tasks": r.json()})
...
def process_task():
    ...
    try:
        ...
        r = requests.get(f"{API_BASE_URL}/tms/task/{task_id}",
                         headers=default_headers({"x-api-key": token}))
        if r.status_code != 200:
            return jsonify({"success": False, "message": "Erro ao buscar detalhes", "detail": r.text}), r.status_code
        ...
        if resp.status_code != 200:
            return jsonify({"success": False, "message": "Erro ao enviar respostas", "detail": resp.text}), resp.status_code
